...
async fn test_completion_anchors() {
    ...
    if result.is_array() {
        let items = result.as_array().unwrap();
        assert!(!items.is_empty(), "⚠️  No anchor completions (workspace scan may still be in progress)");
        let labels: Vec<String> = items
            .iter()
            .filter_map(|item| item["label"].as_str().map(String::from))
            .collect();
        ...
        assert!(
            labels.iter().any(|l| l.contains("intro") || l.contains("#intro")),
