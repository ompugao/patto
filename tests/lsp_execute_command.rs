...
async fn test_aggregate_tasks_empty() {
    ...
    assert!(response.get("result").is_some(), "No result in aggregate_tasks");
    let result = &response["result"];
    assert!(result.is_array() || result.is_null(), "Result should be array or null");
...
async fn test_aggregate_tasks_with_tasks() {
    ...
    workspace.create_file(
        "done.pn",
        "Completed {@task status=done due=2024-12-20}\n",
    );
    ...
    assert!(tasks.len() >= 2, "Should have at least 2 tasks, got {}", tasks.len());
    ...
    for (i, task) in tasks.iter().enumerate() {
        ...
        assert!(task["location"].is_object(), "Task {} should have location object", i);
        assert!(task["location"]["uri"].is_string(), "Task {} location should have uri", i);
        assert!(task["location"]["range"].is_object(), "Task {} location should have range", i);
        assert!(task["text"].is_string(), "Task {} should have text string", i);
        assert!(task["message"].is_string(), "Task {} should have message string", i);
        assert!(task["due"].is_object() || task["due"].is_string(), "Task {} should have due field", i);
    }
    
    // Verify at least one task has expected content
    let task_texts: Vec<&str> = tasks
        .iter()
        .filter_map(|t| t["text"].as_str())
        .collect();
    assert!(
        task_texts.iter().any(|t| t.contains("Task 1") || t.contains("Task 2") || t.contains("Completed")),
...
async fn test_aggregate_tasks_sorting() {
    ...
    let tasks = result.as_array().unwrap();
    assert_eq!(tasks.len(), 3, "Should have exactly 3 tasks, got {}", tasks.len());
    
    // Extract due dates from tasks
    let due_dates: Vec<String> = tasks
        .iter()
        .filter_map(|task| {
            task["due"]["Date"].as_str().map(String::from)
        })
        .collect();
    ...
    assert_eq!(due_dates[0], "2024-01-01", "First task should be Past (2024-01-01)");
    assert_eq!(due_dates[1], "2024-12-25", "Second task should be Soon (2024-12-25)");
    assert_eq!(due_dates[2], "2025-12-31", "Third task should be Future (2025-12-31)");
...
async fn test_two_hop_links_basic() {
    ...
    assert!(response.get("result").is_some(), "No result in two_hop_links");
    let result = &response["result"];
        ...
        if first_link.is_array() {
            ...
            if pair.len() == 2 {
                ...
                if target_url.is_string() {
                    assert!(target_url.as_str().unwrap().contains("target.pn"), 
                ...
                if connected.is_array() {
                    ...
                    assert!(!connected_files.is_empty(), 
                            "Should have connections (other.pn links to target too)");
                    
                    // Verify that other.pn is in the list and source.pn is NOT
                    let urls_str = connected_files.iter()
                        .filter_map(|v| v.as_str())
                        .collect::<Vec<_>>();
                    
                    assert!(urls_str.iter().any(|u| u.contains("other.pn")), 
                            "Should include other.pn");
                    assert!(!urls_str.iter().any(|u| u.contains("source.pn")), 
